    ...
    <div class="home-container">
        ...
        <main class="main-content">
            <section class="hero-section">
                <h2 class="hero-title">Create Schema</h2>
                <p class="hero-subtitle">Select DID, then create a new schema on the ledger.</p>
            </section>
            ...
            <section class="features-section">
                ...
                <div class="feature-card">
                    ...
                    <input v-model="schemaName" type="text" placeholder="e.g. customer_schema" />
                </div>
                <div class="feature-card">
                    <label>Version:</label>
                    <input v-model="schemaVersion" type="text" placeholder="e.g. 1.0" />
                </div>
                <div class="feature-card">
                    <label>Attributes (comma-separated):</label>
                    <input v-model="schemaAttributes" type="text" placeholder="e.g. name, age" />
                </div>
            </section>
            <button @click="createSchema" class="submit-button">Submit Schema</button>

            <section class="features-section" v-if="schemaList.length" style="text-align: center; display: flex; justify-content: center;">
...
export default {
    ...
    methods: {
        ...
        async createSchema() {
            if (!this.selectedMydid || !this.schemaName || !this.schemaVersion || !this.schemaAttributes) {
                alert("Fill all fields");
                return;
            }
            try {
                await api.post("/api/create_schema", {
                    mydid: this.selectedMydid,
                    schema_name: this.schemaName,
                    version: this.schemaVersion,
                    attributes: this.schemaAttributes.split(",").map(attr => attr.trim()),
                });
        ...
        async fetchSchemas() {
            try {
                ...
                if (res.data.status === "success") {
                    // Example: returning an array of schema info
